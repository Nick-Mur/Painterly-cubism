...
def save_image(game=False):
    ...
    if not game:
        ...
        pyautogui.screenshot(f"{nam}{fefe}.png")
...
def play():
    ...
    try:
        ...
        if language_now == 'Eng':
            menu.add.label('Connection problems')
            menu.add.button('Back', main_menu)
        elif language_now == 'Rus':
            menu.add.label('Проблемы с соединенинем')
            menu.add.button('Назад', main_menu)
...
def soon():
    ...
    menu.clear()
    if language_now == 'Rus':
        menu.add.label('Пока в разработке :)')
        menu.add.button('Назад', levels)
    if language_now == 'Eng':
        menu.add.label('Still in development :)')
        menu.add.button('Back', levels)


# уровни игры: дом, замок
# здесь перетаскивание фигур
def house_level():
    menu.disable()
    global undone, maxundone, level
    level = True
    # буквенные переменные равны 1 и вдальнейшем умножаются на 1 и -1, так как отвечают, за наложение фигур друг на
    # друга. То есть можно передвигать фигуру по другой
    c = 1 # отвечает за тело дома и дверь
    d = 0
    tr = 0
    moving = False
    # начальные координаты тела дома
    x, y = 0, 0
    # начальные координаты двери дома
    x1, y1 = 0, 0
    # начальные координаты окна дома
    x2, y2 = 100, 100
    trx1, try1 = 100, 300
    trx2, try2 = 750, 300
    trx3, try3 = 425, 0
    ...
    while run:
        for event in pygame.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_ESCAPE:
                    ...
                    save_image(game=True)
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if x < event.pos[0] < x + 600 and y < event.pos[1] < y + 500:
                    moving = True
                    c *= 1
                if x1 < event.pos[0] < x1 + 200 and y1 < event.pos[1] < y1 + 400:
                    moving = True
                    c *= -1
                if (x2 < event.pos[0] < x2 + 130 or x2 - 130 < event.pos[0] < x2) and \
                        (y2 < event.pos[1] < y2 + 130 or y2 - 130 < event.pos[1] < y2):
                    moving = True
                    c = 0
                    d += 1
                if ((trx1 - event.pos[0]) * (try2 - try1) - (trx2 - trx1) * (try1 - event.pos[1]) >= 0
                        and (trx2 - event.pos[0]) * (try3 - try2) - (trx3 - trx2) * (try2 - event.pos[1]) >= 0
                        and (trx3 - event.pos[0]) * (try1 - try3) - (trx1 - trx3) * (try3 - event.pos[1]) >= 0) or \
                        ((trx1 - event.pos[0]) * (try2 - try1) - (trx2 - trx1) * (try1 - event.pos[1]) <= 0
                     and (trx2 - event.pos[0]) * (try3 - try2) - (trx3 - trx2) * (try2 - event.pos[1]) <= 0
                     and (trx3 - event.pos[0]) * (try1 - try3) - (trx1 - trx3) * (try3 - event.pos[1]) <= 0):
                    moving = True
                    c = 0
                    d = 0
                    tr += 1
            if event.type == pygame.MOUSEMOTION:
                ...
                if moving and d > 0:
                    x_new2, y_new2 = event.rel
                    x2, y2 = x2 + x_new2, y2 + y_new2
                if moving and tr > 0:
                    x_newtr, y_newtr = event.rel
                    trx1, try1 = trx1 + x_newtr, try1 + y_newtr
                    trx2, try2 = trx2 + x_newtr, try2 + y_newtr
                    trx3, try3 = trx3 + x_newtr, try3 + y_newtr
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                ...
                moving = False
        # тело дома
        pygame.draw.rect(window, (230, 164, 133), (x, y, 600, 500))
        # дверь
        pygame.draw.rect(window, (255, 104, 0), (x1, y1, 200, 400))
        # окно
        pygame.draw.circle(window, (60, 170, 255), (x2, y2), 130)
        # крыша
        pygame.draw.polygon(window, (255, 77, 0), ([trx1, try1], [trx2, try2], [trx3, try3]))
        # внешний вид, полосы
        ...
        okno = pg.draw.rect(window, white,
                            (0.75 * width_window + 6.5, 8, 0.25 * width_window - 24, 0.25 * length_window - 24))
        IMAGE = pg.image.load('house.png').convert()
        window.blit(IMAGE, okno)
        # трава
        pg.draw.rect(window, (0, 209, 118),
                     (0, 0.925 * length_window, width_window, 0.075 * length_window))
        pygame.display.flip()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_ESCAPE:
                    ...
                    save_image(game=True)
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if x < event.pos[0] < x + 300 and y < event.pos[1] < y + 100:
                    moving = True
                    c *= 1
                if x1 < event.pos[0] < x1 + 200 and y1 < event.pos[1] < y1 + 700:
                    moving = True
                    c *= -1
                if (x2 < event.pos[0] < x2 + 80 or x2 - 80 < event.pos[0] < x2) and \
                        (y2 < event.pos[1] < y2 + 80 or y2 - 80 < event.pos[1] < y2):
                    moving = True
                    c = 0
                    d += 1
                    tr = 0
                ...
                if ((trx1 - event.pos[0]) * (try2 - try1) - (trx2 - trx1) * (try1 - event.pos[1]) >= 0
                    ...
                    square = 0
                    tr += 1
                if (x3 < event.pos[0] < x3 + 45 or x3 - 45 < event.pos[0] < x3) and \
                        (y3 < event.pos[1] < y3 + 45 or y3 - 45 < event.pos[1] < y3):
                    moving = True
                    c = 0
                    d = 0
                    tr = 0
                    square = 0
                    circle3 += 1
                if x4 < event.pos[0] < x4 + 130 and y4 < event.pos[1] < y4 + 130:
                    moving = True
                    c = 0
                    d = 0
                    tr = 0
            ...
            if event.type == pygame.MOUSEMOTION:
                ...
                if moving and d > 0:
                    x_new2, y_new2 = event.rel
                    x2, y2 = x2 + x_new2, y2 + y_new2
                if moving and circle3 > 0:
                    x_new3, y_new3 = event.rel
                    x3, y3 = x3 + x_new3, y3 + y_new3
                if moving and square > 0:
                    x_new4, y_new4 = event.rel
                    x4, y4 = x4 + x_new4, y4 + y_new4
                if moving and tr > 0:
                    x_newtr, y_newtr = event.rel
                    trx1, try1 = trx1 + x_newtr, try1 + y_newtr
                    trx2, try2 = trx2 + x_newtr, try2 + y_newtr
            ...
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                ...
                square = 0
                moving = False
        pygame.draw.rect(window, (200, 250, 250), (x, y, 300, 100))
        pygame.draw.rect(window, (255, 104, 0), (x1, y1, 200, 700))
        pygame.draw.circle(window, (0, 160, 78), (x2, y2), 80)
        pygame.draw.polygon(window, (205, 77, 100), ([trx1, try1], [trx2, try2], [trx3, try3]))
        pygame.draw.circle(window, (20, 100, 220), (x3, y3), 45)
        pygame.draw.rect(window, (255, 90, 190), (x4, y4, 130, 130))
        pygame.display.flip()
        window.fill((255, 255, 255))
    ...
    try:
        ...
        if c > 0:
            if language_now == 'Eng':
                ...
                menu.add.button('Free level', free_level, font_size=70)
                menu.add.button('Castle', soon, font_size=70)
                ...
                menu.add.button('Свободный уровень', free_level, font_size=70)
                menu.add.button('Замок', soon, font_size=70)
...
def about():
    ...
    if language_now == 'Eng':
        ...
        menu.add.label('The buttons on the screen can be activated by clicking on them with the mouse.', font_size=30)
        ...
        menu.add.label('Pressing the left "Ctrl" while drawing or in the game - minimizes the window.', font_size=30)
        menu.add.label('Pressing the space bar in the game saves the picture to the gallery.', font_size=30)
        ...
        menu.add.label('Кнопки на экране можно активировать с помощью нажатия по ним мыши.', font_size=30)
        ...
        menu.add.label('Нажатие левого "Ctrl" во время рисования или в игре - сворачивает окно.', font_size=30)
        menu.add.label('Нажатие пробела в игре сохраняет картинку в галерею.', font_size=30)
...
def settings():
    ...
    if language_now == 'Eng':
        ...
        if not level:
            ...
            menu.add.button('Enable/Disable the interface', hide_buttons)
        menu.add.button('Back', main_menu)
    elif language_now == 'Rus':
        ...
        if not level:
            ...
            sc = menu.add.selector('Масштабирование фигур: ', [('1', 1), ('2', 2), ('3', 3), ('5', 5), ('10', 10)],
                                   onchange=scaling_shapes)
            menu.add.button('Включить/выключить интерфейс', hide_buttons)
        menu.add.button('Назад', main_menu)
